...
// Chapter verse count data (total verses per chapter)
...
router.get("/gita/chapters/:chapterId", async (req, res) => {
  ...
  // Build verse summaries from cached data + key verses, generate stubs for rest
  const verses = [];
  for (let v = 1; v <= totalVerses; v++) {
    ...
    if (keyVerse) {
      ...
      // Check if we have a cached summary in DB
      ...
      if (cached.length > 0) {
        ...
        // Stub — will be generated on demand when verse page is opened
        verses.push({
          id: v,
          chapterId,
...
Be authentic, scholarly, and faithful to the original meaning. The Sanskrit must be accurate. Themes should be in English.`;
    ...
    // Clean and parse JSON — also handle truncated JSON by attempting partial recovery
...
router.post("/gita/verses/prewarm", async (req, res) => {
  ...
  // Already cached or key verse? skip
  const keyVerse = getKeyVerse(chapterId, verseId);
